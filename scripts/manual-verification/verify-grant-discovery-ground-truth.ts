...
function matchesProgram(program: GroundTruthProgram, discoveredTitle: string, discoveredSummary: string): boolean {
...
async function main() {
  ...
  for (const modificationCode of MODIFICATION_TYPES) {
    ...
    for (const grant of result.discoveredGrants) {
      const program = GROUND_TRUTH_PROGRAMS.find((p) => matchesProgram(p, grant.title, grant.summary));
