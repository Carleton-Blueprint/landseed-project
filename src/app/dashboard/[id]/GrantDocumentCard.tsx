...
export interface GrantDocumentCardProps {
  projectId: string;
  /** Whether the eligibility assessment has reached a final decision (ELIGIBLE or INELIGIBLE).
   *  The summary PDF is only ever generated once this is true, so the card stays hidden until then. */
...
export function GrantDocumentCard({
  ...
  // The summary is generated in the background right after the assessment completes — usually
  // within moments, but poll for it rather than requiring a manual page reload.
  useEffect(() => {
    if (!assessmentComplete || hasDocument) return;
    const interval = setInterval(() => router.refresh(), POLL_INTERVAL_MS);
    return () => clearInterval(interval);
  }, [assessmentComplete, hasDocument, router]);
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <p className="text-sm text-gray-600">
              Your grant eligibility summary PDF is ready for download.
            </p>
            {lastGeneratedAt && (
          ...
          <Link href={`/api/documents/${projectId}/download`} target="_blank" rel="noopener noreferrer">
            <Button variant="default" className="gap-2">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
              </svg>
              Download Grant Eligibility Summary
            </Button>
          </Link>
        </div>
      ) : (
        <div className="flex items-center gap-3 rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4">
          <div className="h-5 w-5 shrink-0 animate-spin rounded-full border-2 border-gray-300 border-t-gray-500" aria-hidden="true" />
          <p className="text-sm text-gray-500">
            Generating your grant eligibility summary… this usually only takes a moment.
          </p>
        </div>
      )}
