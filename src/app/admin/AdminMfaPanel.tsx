...
export function AdminMfaPanel({ currentUserId }: AdminMfaPanelProps) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="px-4 py-3 text-sm font-semibold text-gray-800">Admin MFA Enrollment</div>
                ...
                <li key={admin.id} className="flex items-center justify-between gap-3 py-2.5">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {admin.mfaEnabled
                        ? `Enrolled${
                            admin.mfaEnrolledAt ? ` on ${new Date(admin.mfaEnrolledAt).toLocaleDateString()}` : ""
                          }`
                        : "Not enrolled"}
                    </p>
                  </div>
