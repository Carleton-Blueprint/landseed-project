...
import { PRICING_TIER_LABELS } from './pricingTiers';
...
function drawHeaderField(cursor: EstimatePdfCursor, label: string, value: string): void {
...
function drawLineItem(cursor: EstimatePdfCursor, item: RefinedEstimateLineItem, index: number): void {
  // Reserve enough room for the description + detail line before starting the block,
  // so a line item's header never gets orphaned alone at the bottom of a page.
  cursor.ensureSpace(50);

  cursor.drawLine(`${index + 1}. ${item.description} (qty ${item.quantity})`, {
    size: 12,
    font: cursor.titleFont,
  ...
  cursor.drawLine(
    `Material: ${formatCurrency(item.materialTotal)}   |   Labor: ${formatCurrency(item.laborTotal)}   |   Markup: ${formatCurrency(
      item.markupTotal
...
export async function generateEstimatePdf(input: AssembledEstimateInput): Promise<Buffer> {
  const doc = await PDFDocument.create();
  const page = doc.addPage(PAGE_SIZE);
  const titleFont = await doc.embedFont(StandardFonts.HelveticaBold);
  const bodyFont = await doc.embedFont(StandardFonts.Helvetica);

  const cursor = new EstimatePdfCursor(doc, page, titleFont, bodyFont);

  cursor.drawLine('Landseed - Project Estimate', {
    size: 18,
    font: titleFont,
    color: rgb(0.11, 0.2, 0.37),
  });
  cursor.y -= 8;
  cursor.drawLine('Itemized cost estimate for the requested accessibility modification(s).', {
    size: 10,
    font: bodyFont,
  ...
  if (input.selectedTier) {
    drawHeaderField(cursor, 'Pricing Tier', PRICING_TIER_LABELS[input.selectedTier]);
  }

  cursor.y -= 10;
  cursor.drawLine(`Line Items (${input.pricing.lineItems.length})`, {
    size: 13,
    font: titleFont,
    color: rgb(0.15, 0.15, 0.15),
  });
  ...
  if (input.pricing.lineItems.length === 0) {
    cursor.drawLine('No itemized line items available for this estimate.', {
      size: 11,
      font: bodyFont,
    ...
    input.pricing.lineItems.forEach((item, index) => drawLineItem(cursor, item, index));
  }

  cursor.y -= 10;
  cursor.drawLine('Estimate Summary', { size: 13, font: titleFont, color: rgb(0.15, 0.15, 0.15) });
  cursor.y -= 4;
