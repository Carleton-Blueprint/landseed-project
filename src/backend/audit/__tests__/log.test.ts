...
describe("logAuditEvent", () => {
  ...
  // payloadForHash (used for the eventHash computation) hashes the sanitized
  // (toJsonText-safe) values, not the raw input, specifically so circular-reference
  // metadata can't throw here — see src/backend/audit/log.ts's payloadForHash comment.
  it("does not throw when metadata is circular, and stores a serializationError marker instead", async () => {
    prisma.auditEvent.findFirst.mockResolvedValue(null);
    prisma.$executeRaw.mockResolvedValue(undefined);
    ...
    const sqlArg = prisma.$executeRaw.mock.calls[0][0] as { values: unknown[] };
    expect(sqlArg.values[FIELD_INDEX.metadata]).toBe(JSON.stringify({ serializationError: true }));
    expect(sqlArg.values[FIELD_INDEX.eventHash]).toEqual(expect.any(String));
  });

  it("logAuditEventNonBlocking does not log an error for circular metadata, since logAuditEvent no longer throws for it", async () => {
    prisma.auditEvent.findFirst.mockResolvedValue(null);
    prisma.$executeRaw.mockResolvedValue(undefined);
